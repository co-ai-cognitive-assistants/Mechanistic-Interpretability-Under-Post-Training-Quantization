# quantization/methods/int4.py
# This script implements 4-bit quantization using bitsandbytes.
# It uses the NF4 (4-bit NormalFloat) data type, introduced in the QLoRA paper.
# Source: `bitsandbytes` library by Tim Dettmers.
# Paper: "QLoRA: Efficient Finetuning of Quantized LLMs"
# Link: https://arxiv.org/abs/2305.14314
from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig, AutoConfig, MistralConfig, LlamaTokenizerFast
...
def quantize(model_name, output_dir):
    """
    Quantizes a model to int4 and saves it.
    """
    print(f"Loading and quantizing model to int4: {model_name}")
    ...
    if hasattr(config, "quantization_config"):
        print(f"Warning: Model {model_name} already has a quantization_config. Removing it to proceed with int4 quantization.")
    ...
    print(f"Saving int4 model to: {output_dir}")
